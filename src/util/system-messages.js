...
export function buildSMDString(messageData, renderTimestamp) {
  ...
  // If messageData includes a timestamp...
  if (!!time) {
    ...
    if (renderTimestamp === 'block') {
      return `>t>_t_${clientTime} <>chatfeed-text>` + string();
    };

    if (renderTimestamp === 'inline') {
      return `_t_${clientTime} ^` + string();
    };
  };

  // If no valid `time` or `renderTimestamp` values are given, render string
  // without timestamp.
  return string();
};
